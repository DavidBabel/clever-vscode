...
import { executeMacro, fastCurly, fastArrowFunction, previewCurrentFile } from './controlers/macro';
import { toggleMaximize } from './controlers/maximize';
import { selectEitherQuote, matchingSelect, cleverSelect } from "./services/selector";
...
export function activate(context: ExtensionContext) {
  ...
  const _ = null;
  const r = (s: string, cb: (...args: any[]) => any) => Commands.registerCommand(s, cb);
  ...
  context.subscriptions.push(
    ...
    r('clever.fastJump.small.selectUp', jumpSelectLines.bind(_, jumpSizes.small.up)),
    r('clever.fastJump.small.selectDown', jumpSelectLines.bind(_, jumpSizes.small.down)),
    r('clever.fastJump.big.up', jumpLines.bind(_, jumpSizes.big.up)),
    r('clever.fastJump.big.down', jumpLines.bind(_, jumpSizes.big.down)),
    r('clever.fastJump.big.selectUp', jumpSelectLines.bind(_, jumpSizes.big.up)),
    r('clever.fastJump.big.selectDown', jumpSelectLines.bind(_, jumpSizes.big.down)),
    ...
    r('clever.blockSelect.parenthesis', matchingSelect.bind(_, { start_char: "(", end_char: ")" })),
    r('clever.blockSelect.squareBrackets', matchingSelect.bind(_, { start_char: "[", end_char: "]" })),
    r('clever.blockSelect.curlyBrackets', matchingSelect.bind(_, { start_char: "{", end_char: "}" })),
    r('clever.blockSelect.angleBrackets', matchingSelect.bind(_, { start_char: "<", end_char: ">" })),
    r('clever.blockSelect.inTag', matchingSelect.bind(_, { start_char: ">", end_char: "<" }))
    // clever.calculate
    // clever.block.deploy
    // clever.block.imploy
...
export function deactivate() {
  Window.showInformationMessage('Hope to see you soon, please repport any issue.');
}